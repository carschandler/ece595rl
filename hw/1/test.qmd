...
\left[ R_t |  A_t = a, S_t = s \right] \right] \\
\end{aligned}
$$
...
We assume this hypothesis to be correct for some values $k \le m$ where $k, m \in
\mathbb{N}$. Given that the MC satisfies the Markov property, the following
*base case* holds for $k=m=1$:

$$
\mathbb{P}[X_{t+1} | X_0, X_1, \ldots, X_t] = \mathbb{P}[X_{t+1} | X_t]
$$

We require two induction steps here: one for $m$ and one for $k$. We can begin
with the induction step for $m$:

Assuming the inductive hypothesis to be true, we will show that it is also true
for $m + 1$:

$$
\begin{aligned}
&\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m+1} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m+1} | X_t], \quad \forall t \in
\mathbb{N}_0, \forall k,m \in \mathbb{N}, m \ge k
\end{aligned}
$$

We use the same pattern as before:

$$
\begin{aligned}
&\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m}, X_{t+m+1} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+m+1} | X_0, X_1, \ldots, X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+m+1} | X_0, X_1, \ldots, X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m} | X_t] \\
= &\mathbb{P}[X_{t+m+1} | X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m} | X_t] \\
= &\frac{\mathbb{P}[X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}, X_{t+m+1}]}{\mathbb{P}[X_t X_{t+k}, X_{t+k+1}, \ldots, X_{t+m}]}
\frac{\mathbb{P}[X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t+m}]}{X_t} \\
= &\frac{\mathbb{P}[X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}, X_{t+m+1}]}{X_t} \\
= &\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}, X_{t+m+1} | X_t] \\
\end{aligned}
$$

Thus the induction step for $m$ is proven.

Next we can prove the inductive step over $k$, showing that the inductive
hypothesis is also true for $k+1$:

$$
\begin{aligned}
&\mathbb{P}[X_{t+(k+1)}, X_{t+(k+1)+1}, \ldots, X_{t+m} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+(k+1)}, X_{t+(k+1)+1}, \ldots, X_{t+m} | X_t], \quad \forall t \in
\mathbb{N}_0, \forall k,m \in \mathbb{N}, m \ge k
\end{aligned}
$$
...
$$
\begin{aligned}
&\mathbb{P}[X_{t+k+1}, X_{t+k+2}, \ldots, X_{t+m} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+k+1} | X_0, X_1, \ldots, X_t, X_{t+k+2}, X_{t+k+3}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k+2}, \ldots, X_{t+m} | X_0, X_1, \ldots, X_t] \\
= &\mathbb{P}[X_{t+k+1} | X_0, X_1, \ldots, X_t, X_{t+k+2}, X_{t+k+3}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m} | X_t] \\
= &\mathbb{P}[X_{t+m+1} | X_t, X_{t+k}, X_{t+k+1}, \ldots, X_{t + m}] \cdot
\mathbb{P}[X_{t+k}, X_{t+k+1}, \ldots, X_{t+m} | X_t] \\
...
So, we have proven the induction step for $m+1$, therefore the hypothesis holds
for all $m \in \mathbb{N}$, and the induction hypothesis is proven:
