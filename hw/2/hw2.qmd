...
We can use use the following theorem to determine when we have completed a
sufficient number of iterations:

$$
v^*(s) - v^{\pi_T}(s) \le \frac{2 \gamma^{T + 1}}{1 - \gamma} \Vert v_o - v^*
\Vert_{\infty}, \quad \forall s \in \mathcal{S}
$$

We desire an accuracy of $\varepsilon = 0.01$, which means that we want $|v^*(s) -
v^{\pi_T}| \le 0.01$ for all states. Therefore, if we can get the right hand
side of the equation above to be less than or equal to $0.01$, then we have
achieved our desired accuracy level.

*However*, we do not know $v^*$, which is required to evaluate this equation.
Indeed, this is the quantity we are trying to obtain. Instead, we can use the
estimate for $v^*$ that our algorithm will compute on each iteration, which is
converging to $v^*$, and use it instead. This will give us a bound on our error.
